...
function Cuenta() {
  // seteo del modal en false para que no se muestre aun
  ...
  const SendEmail = (event) => {
    ...
    fetch('./Generate/generate.php', {
      ...
      if (response.ok)
      {
        ...
        //envio mail
        if( (!localStorage.setItem('emailsended')) || (localStorage.setItem('emailsended') !== email) )
        {
          fetch('./Mail/mail.php', {
            method: 'POST',
            headers:{"Content-Type": "application/json" },
  ...
  return (
    ...
    <Modal show={show} onHide={handleClose}>
      ...
      <Modal.Body>¿Deseas qué te enviemos de nuevo el email con la entrada?</Modal.Body>
      <Modal.Footer>
